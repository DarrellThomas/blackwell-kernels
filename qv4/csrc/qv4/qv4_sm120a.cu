...
__constant__ int c_idx_table[6][16] = {
    // pair 0: qubits (0,1)
...
__global__ void qv4_simulate_kernel(
    ...
    float* sre = smem + threadIdx.x * SMEM_STRIDE;
    float* sim = smem + blockDim.x * SMEM_STRIDE + threadIdx.x * SMEM_STRIDE;
...
torch::Tensor qv4_simulate(
    ...
    const int smem_bytes = 2 * threads * SMEM_STRIDE * sizeof(float);
