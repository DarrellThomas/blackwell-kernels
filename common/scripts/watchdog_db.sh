...
mem = ResearchMemory()
state = next((j['state'] for j in mem.get_jobs(kernel_type=kernel) if j['state'] not in terminal), '')
if state:
    print(state)
mem.close()
PY
...
rows.sort(key=lambda j: (int(j['priority']) if str(j['priority']).isdigit() else 99,
                         j['updated_at'], j['id']))
job = rows[0] if rows else None
if not job:
    print(f"No active DB job is assigned to worker family {worker}. Read factory_brain and wait for the next pull from hopper.")
else:
    root = os.environ.get('REPO_ROOT', '')
    common = os.environ.get('COMMON_DIR', root + '/common')
    print(
        f"Resume in this workspace on active job #{job['id']}: {job['title']}. Assume zero prior model context. "
        f"This project is one subdirectory inside the shared {root} factory. The factory database is external to the project tree at {common}/memory/research.db; do not search for a local project database. "
