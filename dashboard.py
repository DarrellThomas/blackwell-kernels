...
function renderLoopStatus(loopStatus) {
  ...
  const colors = { running: '#3fb950', stale: '#d29922', stopped: '#f85149' };
  const sorted = Object.entries(loopStatus).sort((a, b) => a[0].localeCompare(b[0]));
  el.innerHTML = sorted.map(([kernel, info]) => {
    const st = info.status || 'stopped';
    const color = colors[st] || colors.stopped;
    return `<span style="display:inline-flex;align-items:center;gap:4px"><span class="loop-dot ${st}"></span><span style="color:${color}">${kernel}</span></span>`;
  }).join('');
}
